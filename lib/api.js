  ...
  return {
    ...
    flashcards: {
      ...
      updateProgress: (progressData) =>
        api.put(`/flashcards/progress`, progressData),
    ...
    notes: {
      getAll: (collectionId) => api.get(`/collections/${collectionId}/notes`),

      get: (collectionId, noteId) =>
        api.get(`/collections/${collectionId}/notes/${noteId}`),

      create: (collectionId, email, noteData) =>
        api.post(`/notes/${collectionId}/user/${email}`, noteData),
      ...
      update: async (collectionId, noteId, noteData) =>
        api.put(`/notes/${collectionId}/${noteId}`, noteData),
    },
    ...
    tasks: {
      ...
      getByStatus: (workspaceId, status) =>
        api.get(`/tasks/workspace/${workspaceId}/status/${status}`),

      getById: (taskId) => api.get(`/tasks/${taskId}`),

      create: (workspaceId, createdById, taskData) =>
        api.post(
          `/tasks/workspace/${workspaceId}/user/${createdById}`,
          taskData
        ),
    ...
    userStats: {
      getUserStats: async (email) => {
        const response = await api.get(`/user/stats/${email}`);
        return response.data;
      },
      achievementCompleted: async (userId) => {
        const response = await api.post(
          `/user/stats/${userId}/achievement-completed`
        );
        return response.data;
      },
    },
    ...
    userActivity: {
      getRecentActivity: async (userId) => {
        const response = await api.get(`/users/activity/recent/${userId}`);
        return response.data;
      },
      getWeeklyStats: async (userId) => {
        const response = await api.get(`/users/activity/weekly/${userId}`);
        return response.data;
      },
      getMonthlyStats: async (userId) => {
        const response = await api.get(`/users/activity/monthly/${userId}`);
        return response.data;
      },
    },
  };
